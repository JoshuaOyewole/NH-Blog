...
function BlogCard({ ...props }: IArticle) {
  const { title, postedby_name, date_posted, blog_body } = props;
  return (
    <Box as='div' className="flex flex-col">
      <Link href={"#"} className='block rounded-3xl hover:transition-all' >
        ...
        <Heading className='font-roboto-slab text-4xl my-5 font-medium hover:text-[#506fd9ed] transition-all'> {title}</Heading>
      ...
      <Text as='p' className='max-h-[6.7rem] truncate whitespace-normal font-worksans text-[#212529] leading-7 ...' dangerouslySetInnerHTML={{
        __html: `${removeHtmlTags(blog_body.slice(0, 190))}...`,
      }} />
    </Box>
